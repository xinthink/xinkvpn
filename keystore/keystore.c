...
/*
 This file is modified by Yingxin Wu <yingxinwu.g@gmail.com>.
 Removes the uid from the key encoding, to allow key-value pairs stored by 3rd party apps,
 be able to used by system service (such as vpnservice) 
*/
...
static int8_t get()
{
    ...
    encode_key(name, params[0].value, params[0].length);
    LOGI("encoded key is: %s, uid=%d", name, uid);
    int n = decrypt_blob(name, &decryption_key);
    if (n != NO_ERROR) {
        return n;
    }
    send_code(NO_ERROR);
...
static int8_t saw()
{
    ...
    n += encode_key(name, params[0].value, params[0].length);
    send_code(NO_ERROR);
...
static struct user {
    ...
    {~0,           ~0,         TEST | GET | INSERT | DELETE | EXIST | SAW | XINK},
};
